...
def generate_greedy(
    ...
    with torch.no_grad():
        for _ in range(max_length - input_ids.size(1)):
            ...
            next_token_id = (
                torch.argmax(next_token_logits).unsqueeze(0).unsqueeze(0)
            )
...
if __name__ == "__main__":
    ...
    for source in ["story", "json"]:
        ...
        for attempt in tqdm(range(2)):
            ...
            greed_texts[source].append(generated_text)
    
    print("=== Generated Hedgehog Stories ===")
       
    for text in greed_texts["story"]:
        print(text)
    
    print("\n=== Generated JSONs ===")
    
    for text in greed_texts["json"]:
        print(text)
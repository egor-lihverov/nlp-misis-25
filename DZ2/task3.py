...
def generate_sampling_temperature(
    model, tokenizer, input_ids, attention_mask, max_length=1000, eos_token_id=None, temperature=1.0,):
    model.eval()
...
if __name__ == "__main__":
    ...
    for source in ["story", "json"]:
        ...
        for temp in tqdm([0.001, 0.1, 0.5, 1.0, 10.0]):
            generated_text = generate_sampling_temperature(
                model=MODEL,
                tokenizer=TOKENIZER,
                input_ids=input_ids,
            ...
            sampling_temperature_texts[source].append(generated_text)
    
    print("=== Generated Hedgehog Stories ===")
       
    for text in sampling_temperature_texts["story"]:
        print(text)
    
    print("\n=== Generated JSONs ===")
    
    for text in sampling_temperature_texts["json"]:
        print(text)